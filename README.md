...
截至 2026-08-27，Release 测试基线为 **560 / 560 通过**，严格构建为 **0 warning / 0 error**。
...
- 监听 Windows 的最小化开始事件，在主显示器上把窗口快照沿三次贝塞尔曲线飞向对应 Dock 图标。
- 动画固定约 300 ms，并随路径逐渐缩小、淡出；同时限制并发动画窗口数量。
- 快照使用只读屏幕复制，不向目标窗口发送截图消息，也不延迟、取消或替换 Windows 原生最小化。
...
- 启动台支持名称文本和字符子序列搜索，尚未引入拼音索引；部分 UWP 包不提供可直接读取的 Logo 时会显示首字母回退。
