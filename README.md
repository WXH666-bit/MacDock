...
截至 2026-08-28，Release 测试基线为 **579 / 579 通过**，严格构建为 **0 warning / 0 error**。
...
- 临时运行项只存在于内存，永远不会写入 `dock-items.json`；通用系统宿主或无法得到可靠重启目标的进程直接跳过。
...
1. 将 `.lnk` 或 `.exe` 拖到 Dock 上即可固定；未固定的运行应用可右键选择“保留在 Dock”。
...
1. 对 M4 / M5 做受控真机视觉回归，补齐 DPI、缩放、多显示器和不同 UWP 包的兼容性样本。
2. 继续验证托盘第三方应用兼容性；AppBar 在重新设计并通过隔离验证前保持硬关闭。
3. 根据真实使用收益决定是否增加拼音索引、多显示器飞行动画等增强项。
4. 完成功能闭环后补安装、发布、升级和回滚流程。
